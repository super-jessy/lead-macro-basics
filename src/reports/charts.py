...
def build_timeseries_fig(
    ...
    """
    Базовая фигура: лог(S&P) + выбранный макро-индикатор.
    Лаг визуально применяем на фронте (JS), здесь строим стартовую картинку.
    """
    spx = spx_df.copy()
...
def build_heatmap_fig(corr_mat: pd.DataFrame) -> go.Figure:
    """
    Теплокарта корреляций: индикаторы × лаг (месяцы).
    """
    x_vals = corr_mat.columns.tolist()   # логи
    y_vals = corr_mat.index.tolist()     # коды индикаторов
...
def build_ic_fig(code: str, ic_payload: Dict[str, dict]) -> go.Figure:
    """
    Линия IC (corr с next-month return) по лагах для одного индикатора.
    """
